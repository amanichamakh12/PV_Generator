...
import re
from Graph.PipelineMin import extract_chart_with_ollama, extract_chart_with_qwen, extract_chart_with_smolvlm
# from Graph.PipelineMin import extract_chart_with_qwen  # commenté — remplacé par SmolVLM pour test
...
def _assign_agenda_to_slides(slides_data: list[dict]) -> list[dict]:
    # 1. Trouver la slide agenda et extraire les points
    ...
    normalized_items = {_normalize_text(item): item for item in agenda_items}

    # 2. Détecter les slides séparateurs de section.
    # Un séparateur = pas de contenu riche ET le texte complet (titre + contenu combinés)
    # contient un item ODJ en sous-chaîne, avec au plus un préfixe numérique en surplus
    # (ex: slide avec titre="03" et contenu=["Résultats des Calculs PD / LGD / EAD"]).
    section_markers: list[tuple[int, str]] = []
    for idx in range(agenda_slide_idx + 1, len(slides_data)):
        slide = slides_data[idx]

        if slide.get("graphiques") or slide.get("images") or slide.get("tableaux"):
            continue

        all_texts = [
            (slide.get("titre") or "").strip(),
            *((c or "").strip() for c in (slide.get("contenu") or [])),
        ]
        all_texts = [t for t in all_texts if t]
        if not all_texts:
            continue

        full_norm = _normalize_text(" ".join(all_texts))

        for norm_item, orig_item in normalized_items.items():
            if norm_item and norm_item in full_norm:
                # Le reste (après retrait du texte ODJ) doit être minimal :
                # uniquement des chiffres / symboles, pas de mots longs
                remainder = full_norm.replace(norm_item, "").strip()
                if not re.search(r"[a-zà-ÿ]{3,}", remainder):
                    section_markers.append((idx, orig_item))
                    break

    if section_markers:
        for marker_pos, (marker_idx, item) in enumerate(section_markers):
            slides_data[marker_idx]["ordre du jour"] = None  # séparateur lui-même : pas d'ODJ

            end_idx = (
                section_markers[marker_pos + 1][0]
                if marker_pos + 1 < len(section_markers)
                else len(slides_data)
            )
            for i in range(marker_idx + 1, end_idx):
                slides_data[i]["ordre du jour"] = item

        for i in range(agenda_slide_idx + 1, section_markers[0][0]):
            slides_data[i].setdefault("ordre du jour", None)

    else:
        # Fallback : ancrage textuel (présentations sans slides séparateurs).
        # Pour chaque ODJ, trouver la première slide dont le texte complet contient
        # l'item en sous-chaîne → ancre. Toutes les slides suivantes jusqu'à la
        # prochaine ancre appartiennent à cet ODJ.
        ...
        for item in agenda_items:
            ...
            for idx in range(agenda_slide_idx + 1, len(slides_data)):
                slide = slides_data[idx]
                slide_norm = _normalize_text(" ".join(filter(None, [
                    slide.get("titre") or "",
                    " ".join(slide.get("contenu") or []),
                ])))
                if norm_item in slide_norm:
        ...
        if anchors:
            for anchor_pos, (anchor_idx, item) in enumerate(anchors):
                end_idx = (
                    anchors[anchor_pos + 1][0]
                    if anchor_pos + 1 < len(anchors)
                    else len(slides_data)
                )
                for i in range(anchor_idx, end_idx):
                    slides_data[i]["ordre du jour"] = item
            for i in range(agenda_slide_idx + 1, anchors[0][0]):
                slides_data[i].setdefault("ordre du jour", None)
...
def get_pending_images(token: str) -> list[dict]:
    return _pending_images.get(token, [])   # get = lecture seule, ne supprime pas
...
def iter_image_analysis_events(slide_index: int, image_index: int, blob: bytes):
    try:
        # ── Qwen via Ollama (commenté — remplacé par SmolVLM local pour test) ──
        result = extract_chart_with_qwen(blob)

        #result = extract_chart_with_smolvlm(blob)
